...
while true; do
    ...
    case "$choice" in
        ...
        5)
            ...
            case "$fwchoice" in
                1)
                    echo "Memasang ImmortalWrt RC4..."
                    wget -q -O installer http://abidarwi.sh/pakafirmware_immo31125-rc4.sh && \
                    chmod 755 installer && ./installer
                    ;;
                2)
                    echo "Memasang OpenWrt Final..."
                    wget -q -O installer http://abidarwi.sh/pakafirmware_owrt-final.sh && \
                    chmod 755 installer && ./installer
                    ;;
                3)
                    echo "Memasang NSS 30042025..."
                    wget -q -O installer http://abidarwi.sh/pakanss30042025.sh && \
                    chmod 755 installer && ./installer
                    ;;
                4)
                    echo "Memasang NSS 08052025..."
