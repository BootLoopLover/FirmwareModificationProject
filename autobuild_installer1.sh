...
#   Special Thanks to Awiks Telegram Group
...
echo -e "${BLUE}Created by Pakalolo${NC}"
echo -e "${BLUE}Select the firmware distribution you want to build:${NC}"
echo "--------------------------------------------------------"

# === Pilihan Build Mode ===
echo -e "${BLUE}Select build mode:${NC}"
echo "1) Fresh Build (hapus folder lama dan clone ulang)"
echo "2) Rebuild (lanjutkan dari folder sebelumnya)"
echo "--------------------------------------------------------"
...
if [[ "$build_mode" == "1" ]]; then
    echo -e "${BLUE}Performing fresh build: removing existing '${distro}' folder if any...${NC}"
    [ -d "$distro" ] && rm -rf "$distro"
elif [[ "$build_mode" == "2" ]]; then
    if [[ ! -d "$distro" ]]; then
        echo -e "${RED}Folder '$distro' tidak ditemukan. Tidak bisa rebuild.${NC}"
        echo -e "${RED}Gunakan opsi Fresh Build.${NC}"
        exit 1
    else
        echo -e "${GREEN}Melanjutkan build dari folder '$distro' yang sudah ada...${NC}"
        cd "$distro"
        skip_clone=true
    fi
else
